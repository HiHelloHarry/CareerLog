...
> 목표: CareerLog 자동 기록의 정확도와 맥락 풍부도를 경쟁제품 수준으로 끌어올리기
>
> ## 구현 현황
> | Phase | 상태 | 비고 |
> |-------|------|------|
...
| 자연어 검색 | 현재 목적은 "이력서 생성"이지 "과거 검색"이 아님 |
| 전체 화면 녹화 | Rewind 방식은 리소스 과다 + 심리적 저항감 |

**CareerLog의 차별화 전략:**
> Recall/Rewind = "모든 것을 기록하고 나중에 찾기"
> CareerLog = "경력에 필요한 맥락만 가볍게 수집하고, AI가 이력서로 변환"

스크린을 찍지 않으면서도, **창 제목 파싱 + URL + 활동량 + 흐름 감지**로
AI가 정확한 경력 기록을 생성할 수 있는 충분한 맥락을 확보하는 것이 목표.
